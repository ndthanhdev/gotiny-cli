...
async function genFormula(metas) {
	...
	const formula = `class Gotiny < Formula
    desc "Using gotiny.cc from the command-line"
    homepage "https://github.com/ndthanhdev/gotiny-cli"
    version "${ver.replace(/v/g, "")}"
    license "MIT"

    if OS.mac?
      if Hardware::CPU.arm?
        url "https://github.com/ndthanhdev/gotiny-cli/releases/download/${ver}/${
		d.darwin.arm64.binName
	}"
        sha256 "${d.darwin.arm64.hash}"
      else
        url "https://github.com/ndthanhdev/gotiny-cli/releases/download/${ver}/${
		d.darwin.amd64.binName
	}"
        sha256 "${d.darwin.amd64.hash}"
      end
    end
    ...
    if OS.linux?
      if Hardware::CPU.arm? && Hardware::CPU.is_64_bit?
        ...
        url "https://github.com/ndthanhdev/gotiny-cli/releases/download/${ver}/${
		d.linux.amd64.binName
	}"
        sha256 "${d.linux.amd64.hash}"
      else
        url "https://github.com/ndthanhdev/gotiny-cli/releases/download/${ver}/${
		d.linux["386"].binName
	}"
    ...
    def install
      if OS.mac?
        if Hardware::CPU.arm?
          ...
          bin.install "gotiny-${ver}-darwin-amd64" => "gotiny"
        end
      end
      ...
      if OS.linux?
        if Hardware::CPU.arm? && Hardware::CPU.is_64_bit?
          ...
          bin.install "gotiny-${ver}-linux-amd64" => "gotiny"
        else
